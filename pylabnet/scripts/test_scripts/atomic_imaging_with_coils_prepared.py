...
def experiment(**kwargs):
    ...
    sequence_end_ms = max(
        frame_2 + max(camera_ttl_up, aom_pulse_width_ms) + 100,
        imaging_AOM_end + 100,
        mot_coils_end + 100
    )
    ...
    buffer = 0
    ni_clock_handle = NI_card_1.prepare_clock(N=experiment_length_ms + buffer, sample_rate=ni_sample_rate)
    ...
    ni_do_handle = NI_card_2.prepare_tasks()
    ...
    ni_ao_handle = NI_card_3.prepare_tasks()
    ...
    try:
        while thread.running:
            ...
            NI_card_1.start_tasks(ni_clock_handle)
            ...
            NI_card_2.start_tasks(ni_do_handle)
            NI_card_3.start_tasks(ni_ao_handle)
            OPX_client.execute()
            NI_card_2.finalize(ni_do_handle, timeout=120.0, close=False)
            NI_card_3.finalize(ni_ao_handle, timeout=120.0, close=False)
            NI_card_1.stop_tasks(ni_clock_handle)
            NI_card_2.stop_tasks(ni_do_handle)
            NI_card_3.stop_tasks(ni_ao_handle)
        ...
        NI_card_2.stop_tasks(ni_do_handle)
        NI_card_3.stop_tasks(ni_ao_handle)
        for task in ni_clock_handle.tasks.values():
            try:
                task.close()
            except Exception:
                pass
        for task in ni_do_handle.tasks.values():
            try:
                task.close()
            except Exception:
                pass
        for task in ni_ao_handle.tasks.values():
            try:
                task.close()
            except Exception:
                pass
        NI_card_1.clear_stack()
        NI_card_2.clear_stack()
        NI_card_3.clear_stack()
