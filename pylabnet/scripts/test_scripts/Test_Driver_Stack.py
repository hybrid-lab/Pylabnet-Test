...
def experiment(**kwargs):
    ...
    while thread.running:
        ...
        client.build_stack()
        do_el = client.create_new_do_elem(do_channel=1, length=160)# DO NOT MEASURE DIGITAL PULSE DIRECTLY
        client.set_digital_voltage(do_el, simulate=True) # DO NOT MEASURE DIGITAL PULSE DIRECTLY
        # client.set_ao_voltage(pulse = "const", ao_channel = 1, length = 200, amplitude = 0.2,frequency = 50)
        # client.get_ai_voltage(ai_channel=2, ao_channel=1, length=500, time_of_flight=152)
        #client.set_ao_voltage("const", dataset.get_input_parameter('input_channel'), amplitude=0.2)
        #client.set_ao_voltage("const", dataset.get_input_parameter('input_channel'),amplitude = dataset.get_input_parameter('output_voltage'))
