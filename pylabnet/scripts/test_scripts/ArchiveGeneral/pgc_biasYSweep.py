# Bias Y sweep with PGC.
# Full sequence: MOT -> CMOT -> [Dark MOT] -> [PGC] -> release -> TOF(15ms) -> image
# Sweep ao9 (bias Y), other bias coils fixed.
...
INIT_DICT = {
    ...
    # ===================== BIAS Y SWEEP (0 to 1V) =====================
    'bias_1_v': {'Bias Y 1 (V)': '0.0'},
    'bias_2_v': {'Bias Y 2 (V)': '0.2'},
    'bias_3_v': {'Bias Y 3 (V)': '0.4'},
    'bias_4_v': {'Bias Y 4 (V)': '0.6'},
    'bias_5_v': {'Bias Y 5 (V)': '0.8'},
    'bias_6_v': {'Bias Y 6 (V)': '1.0'},
...
def configure(**kwargs):
    ...
    scan_labels = [f"BY={v:.1f}V" for v in bias_values]
...
def experiment(**kwargs):
    ...
    total = len(cam_p)
    logger.info(f"Bias Y sweep: values={bias_values_v}V, tof={tof_us}us, total={total} samples")
    ...
    while thread.running:
        ...
        NI_card_3.set_ao_voltage(ao_channel="ao9", voltages=bias_p, sample_rate=ni_rate)
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, b_v in enumerate(bias_values_v):
                    ...
                    logger.info(
                        f"  BiasY={b_v:.1f}V: n={diff_counts[scan_idx]}, "
