# Bias X field sweep with lattice.
# MOT -> CMOT -> Dark MOT -> PGC -> Lattice ramp -> Lattice hold -> image
# Stationary lattice. Sweep ao8 (bias X) from 0V to 2V.
# Bias Y (ao9) fixed at 0.2V, Bias Z (ao10) fixed at 0V.
...
INIT_DICT = {
    ...
    # ===================== BIAS X SWEEP =====================
    'bias_1_v': {'Bias X 1 (V)': '0.0'},
    'bias_2_v': {'Bias X 2 (V)': '0.4'},
    'bias_3_v': {'Bias X 3 (V)': '0.8'},
    'bias_4_v': {'Bias X 4 (V)': '1.2'},
    'bias_5_v': {'Bias X 5 (V)': '1.6'},
    'bias_6_v': {'Bias X 6 (V)': '2.0'},
    'bias_y_v': {'Bias Y Fixed (V)': '0.2'},
    'bias_z_v': {'Bias Z Fixed (V)': '0.0'},
...
def experiment(**kwargs):
    ...
    bias_z_fixed = float(dataset.get_input_parameter("bias_z_v"))
    ...
    logger.info(
        ...
        f"hold={lat_hold_ms}ms, bias_x_sweep={bias_values_v}V, DIM={dim_freq_mhz:.3f}MHz"
    )
    ...
    def build_sub_sequence(bias_v):
        ...
        # Bias X field: constant throughout entire sequence
        bias = [bias_v] * N

        # Bias Y/Z: fixed constants
        bias_y = [bias_y_fixed] * N
        bias_z = [bias_z_fixed] * N

        return cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, bias, bias_y, bias_z, N
    ...
    for b_v in bias_values_v:
        cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, bias, bias_y, bias_z, N = build_sub_sequence(b_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"Bias X sweep: values={bias_values_v}V, total={total} samples")
    ...
    while thread.running:
        ...
        NI_card_3.set_ao_voltage(ao_channel="ao8", voltages=bias_p, sample_rate=ni_rate)
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, b_v in enumerate(bias_values_v):
                    ...
                    logger.info(
                        f"  BiasX={b_v:+.1f}V: n={diff_counts[scan_idx]}, "
