# Lattice depth sweep.
# MOT -> CMOT -> Dark MOT -> PGC -> Lattice ramp (end voltage swept) -> Lattice hold -> image
# Stationary lattice. Both DIM-3000 at same fixed frequency.
# Sweep lattice ramp end voltage to find optimal depth for loading.
...
INIT_DICT = {
    ...
    # ===================== DEPTH SWEEP =====================
    'depth_1_v': {'Depth 1 End (V)': '-0.3'},
    'depth_2_v': {'Depth 2 End (V)': '0.0'},
    'depth_3_v': {'Depth 3 End (V)': '0.1'},
    'depth_4_v': {'Depth 4 End (V)': '0.3'},
    'depth_5_v': {'Depth 5 End (V)': '0.5'},
    'depth_6_v': {'Depth 6 End (V)': '0.8'},
...
def configure(**kwargs):
    ...
    depth_values = [
        float(dataset.get_input_parameter("depth_1_v")),
        float(dataset.get_input_parameter("depth_2_v")),
        float(dataset.get_input_parameter("depth_3_v")),
        float(dataset.get_input_parameter("depth_4_v")),
        float(dataset.get_input_parameter("depth_5_v")),
        float(dataset.get_input_parameter("depth_6_v")),
    ]
    scan_labels = [f"End={v:.1f}V" for v in depth_values]
    dataset.scan_grid = ScanGridWindow(
        scan_labels=scan_labels,
        diff_levels=DIFF_LEVELS, avg_levels=AVG_DIFF_LEVELS,
        title="Lattice depth sweep (end voltage, hold=35ms)",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"hold={lat_hold_ms}ms, depth_sweep={depth_values_v}V, DIM={dim_freq_mhz:.3f}MHz"
    )
    ...
    def build_sub_sequence(lat_end_v):
    ...
    ldao_p = []
    seg_p = []
    for d_v in depth_values_v:
        cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, N = build_sub_sequence(d_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"Depth sweep: depths={depth_values_v}V, total={total} samples")
    ...
    while thread.running:
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, d_v in enumerate(depth_values_v):
                    ...
                    logger.info(
                        f"  Depth={d_v:.1f}V: n={diff_counts[scan_idx]}, "
