# Bias Z field sweep with lattice.
# MOT -> CMOT -> Dark MOT -> PGC -> Lattice ramp -> Lattice hold -> image
# Stationary lattice. Sweep ao10 (bias Z) from 0V to 10V.
# Bias X (ao8) fixed at 0V, Bias Y (ao9) fixed at 0.2V.
...
INIT_DICT = {
    ...
    # ===================== BIAS Z SWEEP =====================
    'bias_1_v': {'Bias Z 1 (V)': '0.0'},
    'bias_2_v': {'Bias Z 2 (V)': '0.08'},
    'bias_3_v': {'Bias Z 3 (V)': '0.16'},
    'bias_4_v': {'Bias Z 4 (V)': '0.24'},
    'bias_5_v': {'Bias Z 5 (V)': '0.32'},
    'bias_6_v': {'Bias Z 6 (V)': '0.40'},
    'bias_x_v': {'Bias X Fixed (V)': '0.0'},
    'bias_y_v': {'Bias Y Fixed (V)': '0.2'},
...
def experiment(**kwargs):
    ...
    bias_x_fixed = float(dataset.get_input_parameter("bias_x_v"))
    bias_y_fixed = float(dataset.get_input_parameter("bias_y_v"))
    ...
    logger.info(
        ...
        f"dark={dark_ms}ms, pgc={pgc_ms}ms, lat_ramp={lat_ramp_ms}ms, "
        f"hold={lat_hold_ms}ms, bias_z_sweep={bias_values_v}V, DIM={dim_freq_mhz:.3f}MHz"
    )
    ...
    def build_sub_sequence(bias_v):
        ...
        # Bias Z field: constant throughout entire sequence
        bias = [bias_v] * N

        # Bias X/Y: fixed constants
        bias_x = [bias_x_fixed] * N
        bias_y = [bias_y_fixed] * N

        return cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, bias, bias_x, bias_y, N
    ...
    for b_v in bias_values_v:
        cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, bias, bias_x, bias_y, N = build_sub_sequence(b_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"Bias Z sweep: values={bias_values_v}V, total={total} samples")
    ...
    while thread.running:
        ...
        NI_card_3.set_ao_voltage(ao_channel="ao10", voltages=bias_p, sample_rate=ni_rate)
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, b_v in enumerate(bias_values_v):
                    ...
                    logger.info(
                        f"  BiasZ={b_v:+.1f}V: n={diff_counts[scan_idx]}, "
