# PGC detuning sweep with lattice.
# MOT -> CMOT -> Dark MOT -> PGC (VCO swept) -> Lattice ramp -> Lattice hold -> image
# Stationary lattice. Both DIM-3000 at same fixed frequency.
# Sweep PGC detuning to find optimal temperature for lattice loading.
...
INIT_DICT = {
    ...
    # ===================== PGC VCO SWEEP =====================
    'pgc_vco_1': {'PGC VCO 1 (V) [~-70MHz]': '-0.75'},
    'pgc_vco_2': {'PGC VCO 2 (V) [~-83MHz]': '-1.00'},
    'pgc_vco_3': {'PGC VCO 3 (V) [~-96MHz]': '-1.25'},
    'pgc_vco_4': {'PGC VCO 4 (V) [~-109MHz]': '-1.50'},
    'pgc_vco_5': {'PGC VCO 5 (V) [~-122MHz]': '-1.75'},
    'pgc_vco_6': {'PGC VCO 6 (V) [~-135MHz]': '-2.00'},
...
def configure(**kwargs):
    ...
    pgc_vco_values = [
        float(dataset.get_input_parameter("pgc_vco_1")),
        float(dataset.get_input_parameter("pgc_vco_2")),
        float(dataset.get_input_parameter("pgc_vco_3")),
        float(dataset.get_input_parameter("pgc_vco_4")),
        float(dataset.get_input_parameter("pgc_vco_5")),
        float(dataset.get_input_parameter("pgc_vco_6")),
    ]
    scan_labels = [f"VCO={v:.2f}V" for v in pgc_vco_values]
    dataset.scan_grid = ScanGridWindow(
        scan_labels=scan_labels,
        diff_levels=DIFF_LEVELS, avg_levels=AVG_DIFF_LEVELS,
        title="PGC detuning sweep (lattice hold=35ms)",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"hold={lat_hold_ms}ms, pgc_vco_sweep={pgc_vco_values}V, DIM={dim_freq_mhz:.3f}MHz"
    )
    ...
    def build_sub_sequence(pgc_detuning):
    ...
    ldao_p = []
    seg_p = []
    for pgc_v in pgc_vco_values:
        cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, N = build_sub_sequence(pgc_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"PGC sweep: vco={pgc_vco_values}V, total={total} samples")
    ...
    while thread.running:
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, pgc_v in enumerate(pgc_vco_values):
                    ...
                    logger.info(
                        f"  VCO={pgc_v:.2f}V: n={diff_counts[scan_idx]}, "
