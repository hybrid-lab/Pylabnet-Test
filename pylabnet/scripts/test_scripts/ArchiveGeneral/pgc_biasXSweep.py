# Bias X sweep with PGC.
# Full sequence: MOT -> CMOT -> [Dark MOT] -> [PGC] -> release -> TOF(15ms) -> image
# Sweep ao8 (bias X), other bias coils fixed.
...
INIT_DICT = {
    ...
    # ===================== BIAS X SWEEP (0 to 1V) =====================
    'bias_1_v': {'Bias X 1 (V)': '0.0'},
    'bias_2_v': {'Bias X 2 (V)': '0.2'},
    'bias_3_v': {'Bias X 3 (V)': '0.4'},
    'bias_4_v': {'Bias X 4 (V)': '0.6'},
    'bias_5_v': {'Bias X 5 (V)': '0.8'},
    'bias_6_v': {'Bias X 6 (V)': '1.0'},
...
def configure(**kwargs):
    ...
    dataset.scan_grid = ScanGridWindow(
        ...
        title="PGC Bias X sweep (ao8, TOF=15ms)",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"TOF={tof_us}us, bias_x_sweep={bias_values_v}V"
    )
    ...
    total = len(cam_p)
    logger.info(f"Bias X sweep: values={bias_values_v}V, tof={tof_us}us, total={total} samples")
    ...
    while thread.running:
        ...
        NI_card_3.set_ao_voltage(ao_channel="ao10", voltages=[bias_z_fixed] * total, sample_rate=ni_rate)
