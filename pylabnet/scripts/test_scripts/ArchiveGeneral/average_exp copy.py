...
def experiment(**kwargs):
    ...
    while thread.running:
        ...
        dataset.log.error("DATA BATCH: " + repr(rand_pulse))

        client = dataset.OPX_client

        client.build_stack()
        client.set_ao_voltage("arbitrary", dataset.get_input_parameter('input_channel'), wave_function=rand_pulse, length=fuzzy_gauss_len, frequency=0)
