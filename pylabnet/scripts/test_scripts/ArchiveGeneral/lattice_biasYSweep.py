# Bias Y field sweep with lattice.
# MOT -> CMOT -> Dark MOT -> PGC -> Lattice ramp -> Lattice hold -> image
# Stationary lattice. Sweep ao9 (bias Y) from -1V to +1V.
# Optimizes stray field cancellation for PGC and lattice loading.
...
INIT_DICT = {
    ...
    'bias_2_v': {'Bias Y 2 (V)': '0.16'},
    'bias_3_v': {'Bias Y 3 (V)': '0.32'},
    'bias_4_v': {'Bias Y 4 (V)': '0.48'},
    'bias_5_v': {'Bias Y 5 (V)': '0.64'},
    'bias_6_v': {'Bias Y 6 (V)': '0.80'},
...
def configure(**kwargs):
    ...
    dataset.scan_grid = ScanGridWindow(
        ...
        title="Bias Y sweep (ao9, lattice hold=35ms)",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"hold={lat_hold_ms}ms, bias_y_sweep={bias_values_v}V, DIM={dim_freq_mhz:.3f}MHz"
    )
    ...
    def build_sub_sequence(bias_v):
        ...
        # Bias Y field: constant throughout entire sequence
        bias = [bias_v] * N

        return cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, bias, N
    ...
    for b_v in bias_values_v:
        cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, ludo, lddo, luao, ldao, seg, bias, N = build_sub_sequence(b_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"Bias Y sweep: values={bias_values_v}V, total={total} samples")
