# Bias Z sweep with PGC.
# Full sequence: MOT -> CMOT -> [Dark MOT] -> [PGC] -> release -> TOF(15ms) -> image
# Sweep ao10 (bias Z), other bias coils fixed.
# Dark MOT and PGC independently toggleable.
# 6 TOF points, sigma^2 vs t^2 linear fit for temperature.
#
...
INIT_DICT = {
    ...
    # ===================== BIAS Z SWEEP (0 to 1V) =====================
    'bias_1_v': {'Bias Z 1 (V)': '0.0'},
    'bias_2_v': {'Bias Z 2 (V)': '0.2'},
    'bias_3_v': {'Bias Z 3 (V)': '0.4'},
    'bias_4_v': {'Bias Z 4 (V)': '0.6'},
    'bias_5_v': {'Bias Z 5 (V)': '0.8'},
    'bias_6_v': {'Bias Z 6 (V)': '1.0'},
...
def configure(**kwargs):
    ...
    dataset.scan_grid = ScanGridWindow(
        ...
        title="PGC Bias Z sweep (ao10, TOF=15ms)",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"TOF={tof_us}us, bias_z_sweep={bias_values_v}V"
    )
    ...
    for b_v in bias_values_v:
        cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco, N = build_sub_sequence(b_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"Bias Z sweep: values={bias_values_v}V, tof={tof_us}us, total={total} samples")
    ...
    while thread.running:
        ...
        NI_card_3.set_ao_voltage(ao_channel="ao8", voltages=[bias_x_fixed] * total, sample_rate=ni_rate)
        NI_card_3.set_ao_voltage(ao_channel="ao9", voltages=[bias_y_fixed] * total, sample_rate=ni_rate)
