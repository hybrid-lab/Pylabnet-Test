# Dark MOT cooling detuning scan.
# MOT + CMOT phases fixed. Sweep 4 Dark MOT cooling VCO voltages.
# Fixed TOF = 10ms.
#
...
INIT_DICT = {
    ...
    # SCAN: 4 Dark MOT cooling VCO voltages
    'dark_vco_1': {'Dark MOT VCO 1 (V) [-40MHz]': '-0.17'},
    'dark_vco_2': {'Dark MOT VCO 2 (V) [-50MHz]': '-0.36'},
    'dark_vco_3': {'Dark MOT VCO 3 (V) [-55MHz]': '-0.46'},
    'dark_vco_4': {'Dark MOT VCO 4 (V) [-65MHz]': '-0.65'},
...
def configure(**kwargs):
    ...
    scan_vcos = [
        float(dataset.get_input_parameter("dark_vco_1")),
        float(dataset.get_input_parameter("dark_vco_2")),
        float(dataset.get_input_parameter("dark_vco_3")),
        float(dataset.get_input_parameter("dark_vco_4")),
    ]
    # Convert to approx MHz for labels: detuning = (0.6 - V) * 52
    scan_labels = [f"{(0.6 - v)*52:.0f}MHz ({v:.2f}V)" for v in scan_vcos]

    dataset.scan_grid = ScanGridWindow(
        scan_labels=scan_labels,
        diff_levels=DIFF_LEVELS,
        avg_levels=AVG_DIFF_LEVELS,
        title="Dark MOT cooling detuning scan",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"TOF={tof_us}us, F1 at {f1/SPM:.0f}ms, F2 at {f2/SPM:.0f}ms"
    )

    # =========================================================================
    # WAVEFORM CONSTRUCTION -- one sub-sequence per dark VCO scan value
    # =========================================================================
    def build_sub_sequence(dark_vco_v):
        N = f2 + ttl_up + 1
        ...
        # Dark MOT: THIS IS THE SCANNED PARAMETER
        for s in range(dark_s):
            mvco[dark_start + s] = dark_vco_v
    ...
    total = len(cam_p)
    logger.info(
        f"Dark MOT detuning scan: vcos={scan_vcos}, "
        f"dark_power={dark_power}V, dark_rep={dark_rep_power}V, "
    ...
    while thread.running:
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, vco_v in enumerate(scan_vcos):
                    ...
                    logger.info(
                        f"  Dark VCO={vco_v}V: n={diff_counts[scan_idx]}, area={float(avg.sum()):.0f}")
