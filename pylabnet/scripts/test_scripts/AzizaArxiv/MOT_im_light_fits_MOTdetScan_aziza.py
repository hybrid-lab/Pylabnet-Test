...
INIT_DICT = {
    ...
    # Imaging voltage is fixed. The SWEEP scans 4 different loading voltages
    # to find the best MOT detuning for atom number.
    # (NI channels: MOT VCO on ao3, Repump VCO on ao4 — hardcoded.)
    'MOT_freq_VCO_imaging_voltage': {'MOT VCO Imaging Voltage (V)': '0.6'},
    'MOT_freq_VCO_load_1': {'MOT VCO Loading 1 (V)': '0.30'},
    'MOT_freq_VCO_load_2': {'MOT VCO Loading 2 (V)': '0.35'},
    'MOT_freq_VCO_load_3': {'MOT VCO Loading 3 (V)': '0.40'},
    'MOT_freq_VCO_load_4': {'MOT VCO Loading 4 (V)': '0.45'},
...
def configure(**kwargs):
    ...
    scan_voltages = [
        float(dataset.get_input_parameter("MOT_freq_VCO_load_1")),
        float(dataset.get_input_parameter("MOT_freq_VCO_load_2")),
        float(dataset.get_input_parameter("MOT_freq_VCO_load_3")),
        float(dataset.get_input_parameter("MOT_freq_VCO_load_4")),
    ]
    ...
    dataset.scan_grid = ScanGridWindow(
        ...
        title=f"VCO loading scan (TOF={tof_us}us)",
    )
...
def experiment(**kwargs):
    ...
    # scan_voltages (loading voltages) already loaded above.
    ...
    def build_sub_sequence(loading_vco_v):
        ...
        # VCO at the scan's loading voltage for most of the sequence,
        # switches to the fixed imaging voltage around the frame triggers.
        mot_vco = [loading_vco_v] * sub_end
    ...
    logger.info(
        f"Running VCO loading scan: "
        f"tof={tof_us_fixed}us (fixed), "
        f"imaging_vco={MOT_freq_VCO_imaging_voltage}V (fixed), "
        f"loading_scan={scan_voltages}, "
