...
# All phases toggleable. One TOF, one frame pair, continuous update.
...
def experiment(**kwargs):
    ...
    # When CMOT is on: coils off before CMOT ends (Dark MOT is field-free)
    # When no CMOT but Dark MOT: coils off before Dark MOT
    # When only PGC: coils off before PGC
    if dark_enabled:
    ...
    logger.info(
        ...
        f"dark={dark_str}({dark_ms}ms), "
        f"pgc={pgc_str}({pgc_ms}ms), "
