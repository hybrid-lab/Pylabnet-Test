"""
MOT fluorescence imaging script — TOF sweep.

Sweeps 4 TOF values at fixed VCO voltages. Each TOF produces one
signal/background frame pair; all 4 sub-sequences are concatenated
into a single NI waveform. The OPX repeats the waveform for averaging.

Plots: 2 x 4 grid with marginal Gaussian fits (QTimer on main thread).
"""
...
def experiment(**kwargs):
    ...
    def build_sub_sequence(tof_samples):
        frame_1 = mot_loading_time_s + tof_samples
    ...
    for tof_s in tof_samples_list:
        cam, aom, cdo, cao, mvco, sub_end = build_sub_sequence(tof_s)
