...
INIT_DICT = {
    ...
    # MOT coils (fixed, not scanned)
    'mot_coils_on_voltage': {'MOT Coils ON Analog Voltage (V)': '5.5'},
    ...
    # 4 Repump VCO voltages to scan
    'Repump_VCO_1': {'Repump VCO 1 (V)': '-0.2'},
    'Repump_VCO_2': {'Repump VCO 2 (V)': '-0.1'},
    'Repump_VCO_3': {'Repump VCO 3 (V)': '0.0'},
    'Repump_VCO_4': {'Repump VCO 4 (V)': '0.1'},
}
...
def configure(**kwargs):
    ...
    scan_voltages = [
        float(dataset.get_input_parameter("Repump_VCO_1")),
        float(dataset.get_input_parameter("Repump_VCO_2")),
        float(dataset.get_input_parameter("Repump_VCO_3")),
        float(dataset.get_input_parameter("Repump_VCO_4")),
    ]
    scan_labels = [f"Rep={v:.1f}V" for v in scan_voltages]

    dataset.scan_grid = ScanGridWindow(
        scan_labels=scan_labels,
        diff_levels=DIFF_LEVELS,
        avg_levels=AVG_DIFF_LEVELS,
        title=f"Repump VCO scan (TOF={tof_us}us)",
    )
    dataset.graph.hide()
...
def experiment(**kwargs):
    ...
    # 4 Repump VCO voltages to scan
    scan_repump_voltages = [
        float(dataset.get_input_parameter("Repump_VCO_1")),
        float(dataset.get_input_parameter("Repump_VCO_2")),
        float(dataset.get_input_parameter("Repump_VCO_3")),
        float(dataset.get_input_parameter("Repump_VCO_4")),
    ]
    n_scan = len(scan_repump_voltages)
    ...
    # Repump VCO voltages are in scan_repump_voltages (loaded above)
    ...
    coils_off_time_s = mot_loading_time_s - 2 * SAMPLES_PER_MS  # TTL 2ms early
    ...
    # =========================================================================
    def build_sub_sequence(repump_vco_v):
        ...
        # Coils DO: off 2ms early, back on 10ms after frame_1
        coils_restore_s = frame_1 + 10 * SAMPLES_PER_MS
        coils_do_arr = [0] * sub_end
        for idx in range(coils_off_time_s, min(coils_restore_s, sub_end)):
            coils_do_arr[idx] = 1

        # Coils analog: fixed at mot_coils_on_voltage
        coils_ao_arr = [mot_coils_on_voltage] * sub_end

        # MOT VCO: loading voltage during MOT, imaging voltage around frames
        ...
        # Repump VCO: THIS is what varies per scan point
        repump_vco = [repump_vco_v] * sub_end

        return cam, aom, coils_do_arr, coils_ao_arr, mot_vco, repump_vco, sub_end
    ...
    for rep_v in scan_repump_voltages:
        cam, aom, cdo, cao, mvco, rvco, sub_end = build_sub_sequence(rep_v)
    ...
    logger.info(
        f"Running Repump VCO scan: "
        f"tof={tof_us_fixed}us (fixed), "
        f"coils={mot_coils_on_voltage}V (fixed), "
        f"repump_scan={scan_repump_voltages}, "
    ...
    while thread.running:
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, rep_v in enumerate(scan_repump_voltages):
                    ...
                    logger.info(
                        f"  Repump={rep_v}V: n_avg={diff_counts[scan_idx]}, area={total_area:.0f}")
