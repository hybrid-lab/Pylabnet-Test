# Lattice + BIAS Z SWEEP.
# Full sequence:
#   MOT -> CMOT -> Dark MOT -> PGC_cool -> PGC_rampdown(+lattice loading)
#   -> lattice HOLD (FIXED 20ms) -> release -> short TOF -> image
# Sweep ao8/ao9/ao10 = Bias Z from 0 to 1V in 10 points.
# Other two bias axes fixed via GUI.
...
INIT_DICT = {
    ...
    # ===================== FIXED LATTICE HOLD =====================
    'hold_ms': {'Lattice Hold (ms)': '20'},

    # ===================== FIXED TOF after release =====================
    'tof_us': {'TOF (us)': '2000'},

    # ===================== BIAS Z SWEEP (10 points, 0-1V) =====================
    'bias_1_v': {'Bias Z 1 (V)': '0.000'},
    'bias_2_v': {'Bias Z 2 (V)': '0.333'},
    'bias_3_v': {'Bias Z 3 (V)': '0.667'},
    'bias_4_v': {'Bias Z 4 (V)': '1.000'},
    'bias_5_v': {'Bias Z 5 (V)': '1.333'},
    'bias_6_v': {'Bias Z 6 (V)': '1.667'},
    'bias_7_v': {'Bias Z 7 (V)': '2.000'},
    'bias_8_v': {'Bias Z 8 (V)': '2.333'},
    'bias_9_v': {'Bias Z 9 (V)': '2.667'},
    'bias_10_v': {'Bias Z 10 (V)': '3.000'},
    ...
    # ===================== FIXED BIASES (X, Y) =====================
    # Bias X and Y held at these values for the entire sub-sequence;
    # Bias Z is swept (see BIAS Z SWEEP block above).
    'bias_x_fixed_v': {'Bias X Fixed (V)': '0.0'},
    'bias_y_fixed_v': {'Bias Y Fixed (V)': '0.44'},
...
def experiment(**kwargs):
    ...
    hold_ms = int(dataset.get_input_parameter("hold_ms"))
    ...
    bias_y_fixed = float(dataset.get_input_parameter("bias_y_fixed_v"))
    ...
    logger.info(
        ...
        f"lattice HOLD = {hold_ms}ms, TOF = {tof_us}us, "
        f"sweep Bias Z = {bias_values_v}V"
    )
    ...
    def build_sub_sequence(bias_swept_v):
        release = pgc_end + hold_s_fixed  # release happens AFTER lattice hold
        ...
        # Bias coils: held CONSTANT at swept / fixed values for the ENTIRE
        # sub-sequence (no ramping, on through MOT loading too).
        biasx = [bias_x_fixed] * N
        biasy = [bias_y_fixed] * N
        biasz = [bias_swept_v] * N
    ...
    for b_v in bias_values_v:
        (cam, ado, aao, rdo, rao, cdo, cao, mvco, rvco,
         bx_w, by_w, bz_w, ludo, lddo, luao, ldao, N) = build_sub_sequence(b_v)
        cam_p += cam
        ado_p += ado
        aao_p += aao
        rdo_p += rdo
    ...
    total = len(cam_p)
    logger.info(f"Lattice + Bias Z sweep: bias_v={bias_values_v}, hold={hold_ms}ms, tof={tof_us}us, total={total} samples")
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, b_v in enumerate(bias_values_v):
                    ...
                    logger.info(
                        f"  BiasZ={b_v:.2f}V: n={diff_counts[scan_idx]}, "
