...
INIT_DICT = {
    ...
    'frame_5': {'Camera Frame 5 Time (ms)': '720'},
    'frame_6': {'Camera Frame 6 Time (ms)': '725'},
...
def configure(**kwargs):
    ...
    # 7th picture: subtraction image
    # 8th picture: running average of subtraction
    dataset.add_child(
    ...
    dataset.children["Frame 6"].graph.setLevels(*FRAME6_LEVELS)
...
def experiment(**kwargs):
    ...
    frame_times = [
        ...
        int(dataset.get_input_parameter("frame_6")),
    ]
    ...
    # Build camera trigger pulse with 6 triggers
    ...
    while thread.running:
        ...
        # 7th image = subtraction of first and sixth raw frames
        diff = frames[0].astype(np.int32) - frames[5].astype(np.int32)

        # Update first 6 raw-frame plots
        for i in range(6):
            frame_ds = dataset.children[f"Frame {i + 1}"]
            frame_ds.set_data(frames[i])
            frame_ds.update()

        # Update 7th plot (difference)
        diff_ds = dataset.children["Image Difference"]
        ...
        # 8th plot = running average of the difference image
