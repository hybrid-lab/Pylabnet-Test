...
INIT_DICT = {
    ...
    # ===================== BIAS X SWEEP (10 points, 0-1V) =====================
    'bias_1_v': {'Bias X 1 (V)': '0.000'},
    'bias_2_v': {'Bias X 2 (V)': '0.049'},
    'bias_3_v': {'Bias X 3 (V)': '0.098'},
    'bias_4_v': {'Bias X 4 (V)': '0.147'},
    'bias_5_v': {'Bias X 5 (V)': '0.196'},
    'bias_6_v': {'Bias X 6 (V)': '0.244'},
    'bias_7_v': {'Bias X 7 (V)': '0.293'},
    'bias_8_v': {'Bias X 8 (V)': '0.342'},
    'bias_9_v': {'Bias X 9 (V)': '0.391'},
    'bias_10_v': {'Bias X 10 (V)': '0.440'},
    ...
    # ===================== FIXED BIASES (Y, Z) =====================
    # Bias Y and Z held at these values for the entire sub-sequence;
    # Bias X is swept (see BIAS X SWEEP block above).
    'bias_y_fixed_v': {'Bias Y Fixed (V)': '0.0'},
...
def experiment(**kwargs):
    ...
    logger.info(
        ...
        f"sweep Bias X = {bias_values_v}V"
    )
    ...
    def build_sub_sequence(bias_swept_v):
        ...
        biasx = [bias_swept_v] * N
        biasy = [bias_y_fixed] * N
        biasz = [bias_z_fixed] * N
    ...
    total = len(cam_p)
    logger.info(f"Lattice + Bias X sweep: bias_v={bias_values_v}, hold={hold_ms}ms, tof={tof_us}us, total={total} samples")
        ...
        try:
            for run_idx in range(M):
                ...
                for scan_idx, b_v in enumerate(bias_values_v):
                    ...
                    logger.info(
                        f"  BiasX={b_v:.2f}V: n={diff_counts[scan_idx]}, "
