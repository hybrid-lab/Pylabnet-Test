# 256mm lattice TRANSPORT (top -> bottom chamber).
# Atoms loaded in top chamber, transported 256 mm via moving lattice,
# imaged with BOTTOM camera (separate pylabnet client) after move.
# Sweeps post-move HOLD TIME (0, 7, 13, 20 ms) to find the right delay
# between move-end and bottom-cam image.
#
# Full sequence per scan point:
#   MOT (top) -> CMOT -> Dark MOT -> PGC_cool -> PGC_rampdown(+lattice load)
#   -> pre-move hold (5ms)
#   -> ACCELERATE (DIM seg 1: 20ms ramp, base -> plateau_freq)
#   -> CONSTANT VELOCITY plateau (~160ms -> 256mm of motion)
#   -> DECELERATE (DIM seg 3: 20ms ramp, plateau -> base)
#   -> POST-MOVE HOLD (swept: 0..20ms)
#   -> BOTTOM cam frame 1 (atoms in lattice at bottom) - same imaging
#      MOT/repump power, same imaging VCO, same exposure as top cam.
#   -> long clear (atoms gone)
#   -> BOTTOM cam frame 2 (background)
# Move budget: 256mm in 200ms total => v_peak ~ 1422mm/s, df ~ 3.58 MHz, ~8g
#
...
INIT_DICT = {
    ...
    # ===================== MOVE PARAMETERS (fixed, single 256mm move) =====================
    # Trapezoidal: accel (move_ramp_ms) -> plateau -> decel (move_ramp_ms).
    # Plateau duration computed automatically from requested distance.
    'transport_distance_mm': {'Transport Distance (mm)': '256.0'},
    'move_ramp_ms': {'Move Accel/Decel Ramp (ms)': '20'},
    'move_total_ms': {'Total Move Time (ms)': '200'},
    'seg_lead_ms': {'DIM3000 Trigger Lead (ms)': '3'},
    # +1 or -1: which sign of detuning -> reverses lattice motion direction.
    # Set this so atoms move FROM top TO bottom chamber.
    'move_direction': {'Move Direction (+1 or -1)': '1'},

    # ===================== POST-MOVE HOLD SWEEP (4 points) =====================
    # Atoms reach bottom; we hold them in the lattice for variable time
    # before imaging with bottom cam. Sweep helps find when atoms arrive
    # (and how quickly they decay after arrival).
    'post_hold_1_ms': {'Post-move Hold 1 (ms)': '0'},
    'post_hold_2_ms': {'Post-move Hold 2 (ms)': '7'},
    'post_hold_3_ms': {'Post-move Hold 3 (ms)': '13'},
    'post_hold_4_ms': {'Post-move Hold 4 (ms)': '20'},

    # ===================== DIM-3000 AOM DRIVER =====================
    'dim_base_freq_mhz': {'DIM3000 Base Freq (MHz)': '100.000'},
    'dim_amplitude_dbm': {'DIM3000 Amplitude (dBm)': '32.0'},

    # ===================== TOF after move (default 0 = image in lattice) =====================
    'tof_us': {'TOF (us)': '0'},
...
def _resolve_bottom_camera_client(kwargs):
    return kwargs["fluorescence_imaging_camera_bfs_u3_51s5m_bottom_chamber"]
...
def configure(**kwargs):
    ...
    dataset.bottom_camera_client = _resolve_bottom_camera_client(kwargs)
    post_hold_values_ms = [
    ...
    dataset.scan_grid = ScanGridWindow(
        ...
        title="Top->Bottom Transport (256mm, post-move hold sweep)",
    )
...
def experiment(**kwargs):
    ...
    bottom_camera_client = _resolve_bottom_camera_client(kwargs)
    ...
    transport_distance_mm = float(dataset.get_input_parameter("transport_distance_mm"))
    move_ramp_ms = int(dataset.get_input_parameter("move_ramp_ms"))
    move_total_ms = int(dataset.get_input_parameter("move_total_ms"))
    # Plateau duration derived from total - 2*ramp
    move_plateau_ms = max(1, move_total_ms - 2 * move_ramp_ms)
    ...
    # Trapezoid: distance = v_peak * (ramp + plateau)
    # Solve for v_peak from requested transport distance.
    move_traveled_ms = move_ramp_ms + move_plateau_ms
    v_peak_mmps = transport_distance_mm / (move_traveled_ms * 1e-3)  # mm/s
    ...
    logger.info(
        f"Transport: distance={transport_distance_mm}mm, "
        f"ramp={move_ramp_ms}ms, plateau={move_plateau_ms}ms, total={move_total_ms}ms, "
        f"v_peak={v_peak_mmps:.1f}mm/s, detune={df_hz/1e6:+.3f}MHz, accel={accel_g:.1f}g")
    ...
    def _dim_program_segments(ser, move_freq):
        ...
        Same ramp time, same endpoint for all scan points in this experiment
        (post-hold sweep doesn't change the move itself). Re-asserts Mseg:1."""
        freq_range = abs(move_freq - base_freq_hz)
        # For very large freq ranges (MHz-scale ramps), use more steps
        max_step_us = 256.0
        min_steps = max(1, int(np.ceil(move_ramp_ms * 1000.0 / max_step_us)))
        num_steps = max(min_steps, 100)   # ~100 steps for smooth MHz-scale ramps
    ...
    if ser_sweep is not None and ser_fixed is not None:
        ...
        # Program the (single) transport segments. Same freq for all scan points.
    ...
    logger.info(
        ...
        f"plateau={move_plateau_ms}ms, decel={move_ramp_ms}ms (TOTAL move={move_total_ms}ms), "
    ...
    def build_sub_sequence(post_hold_ms_local):
        ...
        #   pre_hold -> accel -> plateau -> decel -> post_hold(sweep) -> image
        # Move is always the full 256mm; only the post-hold varies per scan.
        ramp_s = move_ramp_s
    ...
    total = len(cam_p)
    logger.info(f"Top->Bottom Transport: post-hold sweep={post_hold_values_ms}ms, "
                f"transport={transport_distance_mm}mm in {move_total_ms}ms, total={total} samples")
    ...
    while thread.running:
        # Configure BOTTOM camera for hardware-triggered acquisition. The bottom
        # cam will be triggered via dio7 (not dio0). dio0 is left disabled
        # below (the top cam doesn't fire in this experiment).
        ...
        # Top camera DISABLED for this experiment (atoms have moved away).
        NI_card_2.set_do_voltage(do_channel=camera_trigger_do, value=[0] * total, sample_rate=ni_rate)
        # BOTTOM camera trigger goes on dio7. Same TTL pattern as 'cam_p'.
        ...
        try:
            ...
            for run_idx in range(M):
                ...
                for scan_idx, ph_v in enumerate(post_hold_values_ms):
                    ...
                    logger.info(
                        f"  post-hold={ph_v}ms (BOTTOM cam): "
            ...
            logger.info("Stopping acquisition (BOTTOM camera)")
